...
with
    int_results as (select * from {{ ref("int_results") }}),
    ...
    base_results as (
        select
            ...
            status
        from int_results
        left join circuits on int_results.circuit_id = circuits.circuit_id
        left join
            int_pit_stops
            on int_results.driver_id = int_pit_stops.driver_id
            and int_results.race_id = int_pit_stops.race_id
    )
